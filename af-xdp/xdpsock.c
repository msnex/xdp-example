...
static void dump_app_stats(long dt)
{
    ...
    for (i = 0; i < num_socks && xsks[i]; i++) {
        ...
        double rx_empty_polls_ps, fill_fail_polls_ps, copy_tx_sendtos_ps, tx_wakeup_sendtos_ps, opt_polls_ps;

        rx_empty_polls_ps =
            (xsks[i]->app_stats.rx_empty_polls - xsks[i]->app_stats.prev_rx_empty_polls) * 1000000000. / dt;
        fill_fail_polls_ps =
            (xsks[i]->app_stats.fill_fail_polls - xsks[i]->app_stats.prev_fill_fail_polls) * 1000000000. / dt;
        copy_tx_sendtos_ps =
            (xsks[i]->app_stats.copy_tx_sendtos - xsks[i]->app_stats.prev_copy_tx_sendtos) * 1000000000. / dt;
        tx_wakeup_sendtos_ps =
            (xsks[i]->app_stats.tx_wakeup_sendtos - xsks[i]->app_stats.prev_tx_wakeup_sendtos) * 1000000000. / dt;
        opt_polls_ps = (xsks[i]->app_stats.opt_polls - xsks[i]->app_stats.prev_opt_polls) * 1000000000. / dt;

        printf("\n%-18s %-14s %-14s\n", "", "calls/s", "count");
        printf(fmt, "rx empty polls", rx_empty_polls_ps, xsks[i]->app_stats.rx_empty_polls);
        printf(fmt, "fill fail polls", fill_fail_polls_ps, xsks[i]->app_stats.fill_fail_polls);
        printf(fmt, "copy tx sendtos", copy_tx_sendtos_ps, xsks[i]->app_stats.copy_tx_sendtos);
        printf(fmt, "tx wakeup sendtos", tx_wakeup_sendtos_ps, xsks[i]->app_stats.tx_wakeup_sendtos);
    ...
    if (opt_tx_cycle_ns) {
        ...
        printf("%-18s %-10lu %-10lu %-10lu %-10lu %-10lu\n", "Cyclic TX", opt_tx_cycle_ns, tx_cycle_diff_min,
               (long)(tx_cycle_diff_ave / tx_cycle_cnt), tx_cycle_diff_max, tx_cycle_cnt);
    }
}
...
static bool get_interrupt_number(void)
{
    ...
    while (!feof(f_int_proc) && !found) {
        ...
        if (fgets(line, sizeof(line), f_int_proc) == NULL || line[strlen(line) - 1] != '\n') {
...
static int get_irqs(void)
{
    ...
    snprintf(count_path, sizeof(count_path), "/sys/kernel/irq/%i/per_cpu_count", irq_no);
...
static void dump_driver_stats(long dt)
{
    ...
    for (i = 0; i < num_socks && xsks[i]; i++) {
        ...
        intrs_ps = (xsks[i]->drv_stats.intrs - xsks[i]->drv_stats.prev_intrs) * 1000000000. / dt;
...
static void dump_stats(void)
{
    ...
    for (i = 0; i < num_socks && xsks[i]; i++) {
        char *fmt = "%-18s %'-14.0f %'-14lu\n";
        double rx_pps, tx_pps, dropped_pps, rx_invalid_pps, full_pps, fill_empty_pps, tx_invalid_pps, tx_empty_pps;

        rx_pps = (xsks[i]->ring_stats.rx_npkts - xsks[i]->ring_stats.prev_rx_npkts) * 1000000000. / dt;
        tx_pps = (xsks[i]->ring_stats.tx_npkts - xsks[i]->ring_stats.prev_tx_npkts) * 1000000000. / dt;
        ...
        if (opt_frags) {
            ...
            double rx_fps = (rx_frags - xsks[i]->ring_stats.prev_rx_frags) * 1000000000. / dt;
            double tx_fps = (tx_frags - xsks[i]->ring_stats.prev_tx_frags) * 1000000000. / dt;
            char *ffmt = "%-18s %'-14.0f %'-14lu %'-14.0f %'-14lu\n";

            printf("%-18s %-14s %-14s %-14s %-14s %-14.2f\n", "", "pps", "pkts", "fps", "frags", dt / 1000000000.);
        ...
        if (opt_extra_stats) {
            if (!xsk_get_xdp_stats(xsk_socket__fd(xsks[i]->xsk), xsks[i])) {
                dropped_pps = (xsks[i]->ring_stats.rx_dropped_npkts - xsks[i]->ring_stats.prev_rx_dropped_npkts) *
                              1000000000. / dt;
                rx_invalid_pps = (xsks[i]->ring_stats.rx_invalid_npkts - xsks[i]->ring_stats.prev_rx_invalid_npkts) *
                                 1000000000. / dt;
                tx_invalid_pps = (xsks[i]->ring_stats.tx_invalid_npkts - xsks[i]->ring_stats.prev_tx_invalid_npkts) *
                                 1000000000. / dt;
                full_pps =
                    (xsks[i]->ring_stats.rx_full_npkts - xsks[i]->ring_stats.prev_rx_full_npkts) * 1000000000. / dt;
                fill_empty_pps =
                    (xsks[i]->ring_stats.rx_fill_empty_npkts - xsks[i]->ring_stats.prev_rx_fill_empty_npkts) *
                    1000000000. / dt;
                tx_empty_pps =
                    (xsks[i]->ring_stats.tx_empty_npkts - xsks[i]->ring_stats.prev_tx_empty_npkts) * 1000000000. / dt;

                printf(fmt, "rx dropped", dropped_pps, xsks[i]->ring_stats.rx_dropped_npkts);
                printf(fmt, "rx invalid", rx_invalid_pps, xsks[i]->ring_stats.rx_invalid_npkts);
                printf(fmt, "tx invalid", tx_invalid_pps, xsks[i]->ring_stats.tx_invalid_npkts);
                printf(fmt, "rx queue full", full_pps, xsks[i]->ring_stats.rx_full_npkts);
                printf(fmt, "fill ring empty", fill_empty_pps, xsks[i]->ring_stats.rx_fill_empty_npkts);
                printf(fmt, "tx ring empty", tx_empty_pps, xsks[i]->ring_stats.tx_empty_npkts);

                xsks[i]->ring_stats.prev_rx_dropped_npkts = xsks[i]->ring_stats.rx_dropped_npkts;
                xsks[i]->ring_stats.prev_rx_invalid_npkts = xsks[i]->ring_stats.rx_invalid_npkts;
                xsks[i]->ring_stats.prev_tx_invalid_npkts = xsks[i]->ring_stats.tx_invalid_npkts;
                xsks[i]->ring_stats.prev_rx_full_npkts = xsks[i]->ring_stats.rx_full_npkts;
                xsks[i]->ring_stats.prev_rx_fill_empty_npkts = xsks[i]->ring_stats.rx_fill_empty_npkts;
                xsks[i]->ring_stats.prev_tx_empty_npkts = xsks[i]->ring_stats.tx_empty_npkts;
            } else {
...
#define ETH_HDR_SIZE      (opt_vlan_tag ? sizeof(struct vlan_ethhdr) : sizeof(struct ethhdr))
...
#define PKTGEN_SIZE_MIN   (PKTGEN_HDR_OFFSET + sizeof(struct pktgen_hdr) + ETH_FCS_SIZE)
...
static void gen_eth_hdr_data(void)
{
    ...
    if (opt_vlan_tag) {
        ...
        udp_hdr = (struct udphdr *)(pkt_data + sizeof(struct vlan_ethhdr) + sizeof(struct iphdr));
        ip_hdr = (struct iphdr *)(pkt_data + sizeof(struct vlan_ethhdr));
        pktgen_hdr =
            (struct pktgen_hdr *)(pkt_data + sizeof(struct vlan_ethhdr) + sizeof(struct iphdr) + sizeof(struct udphdr));
        ...
        pktgen_hdr =
            (struct pktgen_hdr *)(pkt_data + sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct udphdr));
        /* ethernet header */
    ...
    /* UDP data */
    memset32_htonl(pkt_data + PKT_HDR_SIZE, opt_pkt_fill_pattern, UDP_PKT_DATA_SIZE);

    /* UDP header checksum */
    udp_hdr->check = 0;
    udp_hdr->check = udp_csum(ip_hdr->saddr, ip_hdr->daddr, UDP_PKT_SIZE, IPPROTO_UDP, (u16 *)udp_hdr);
}
...
static void gen_eth_frame(struct xsk_umem_info *umem, u64 addr)
{
    ...
    if (len < opt_xsk_frame_size)
        copy_len = len;
    memcpy(xsk_umem__get_data(umem->buffer, addr), pkt_data + PKT_SIZE - len, copy_len);
    len -= copy_len;
}
...
static struct xsk_socket_info *xsk_configure_socket(struct xsk_umem_info *umem, bool rx, bool tx)
{
    ...
    rxr = rx ? &xsk->rx : NULL;
    txr = tx ? &xsk->tx : NULL;
    ret = xsk_socket__create(&xsk->xsk, opt_if, opt_queue, umem->umem, rxr, txr, &cfg);
    if (ret)
        exit_with_error(-ret);
...
static struct option long_options[] = {{"rxdrop", no_argument, 0, 'r'},
...
static void usage(const char *prog)
{
    const char *str =
        ...
        "\n";
    fprintf(stderr, str, prog, XSK_UMEM__DEFAULT_FRAME_SIZE, opt_batch_size, MIN_PKT_SIZE, MIN_PKT_SIZE, MAX_PKT_SIZE,
            opt_pkt_fill_pattern, VLAN_VID__DEFAULT, VLAN_PRI__DEFAULT, SCHED_PRI__DEFAULT);
...
static void parse_command_line(int argc, char **argv)
{
    ...
    for (;;) {
        c = getopt_long(argc, argv, "rtli:q:pSNn:w:O:czf:muMd:b:C:s:P:VJ:K:G:H:T:yW:U:xQaI:BRF", long_options,
                        &option_index);
        if (c == -1)
            break;
        ...
        case 'w':
            if (get_clockid(&opt_clock, optarg)) {
                fprintf(stderr, "ERROR: Invalid clock %s. Default to CLOCK_MONOTONIC.\n", optarg);
        ...
        case 's':
            ...
            if (opt_pkt_size > (MAX_PKT_SIZE) || opt_pkt_size < MIN_PKT_SIZE) {
        ...
        case 'K':
            ...
            break;
        case 'G':
            if (!ether_aton_r(optarg, (struct ether_addr *)&opt_txdmac)) {
                fprintf(stderr, "Invalid dmac address:%s\n", optarg);
                usage(basename(argv[0]));
            }
            break;
        case 'H':
            if (!ether_aton_r(optarg, (struct ether_addr *)&opt_txsmac)) {
        ...
        case 'W':
            if (get_schpolicy(&opt_schpolicy, optarg)) {
                fprintf(stderr, "ERROR: Invalid policy %s. Default to SCHED_OTHER.\n", optarg);
...
static inline void complete_tx_l2fwd(struct xsk_socket_info *xsk)
{
    ...
    ndescs = (xsk->outstanding_tx > opt_batch_size) ? opt_batch_size : xsk->outstanding_tx;
    ...
    if (rcvd > 0) {
        ...
        for (i = 0; i < rcvd; i++)
            *xsk_ring_prod__fill_addr(&umem->fq, idx_fq++) = *xsk_ring_cons__comp_addr(&umem->cq, idx_cq++);
...
static void tx_only_all(void)
{
    ...
    if (opt_poll && opt_tx_cycle_ns) {
        fprintf(stderr, "Error: --poll and --tx-cycles are both set\n");
        return;
    }
    ...
    if (opt_tx_cycle_ns) {
        ...
        next_tx_ns = (get_nsecs() / NSEC_PER_USEC + 1) * NSEC_PER_USEC;
    ...
    while ((opt_pkt_count && pkt_cnt < opt_pkt_count) || !opt_pkt_count) {
        ...
        if (opt_tx_cycle_ns) {
            ...
            if (err) {
                if (err != EINTR)
                    fprintf(stderr, "clock_nanosleep failed. Err:%d errno:%d\n", err, errno);
                break;
            }
...
static int lookup_bpf_map(int prog_fd)
{
    ...
    for (i = 0; i < prog_info.nr_map_ids; i++) {
        ...
        if (!strncmp(map_info.name, "xsks_map", sizeof(map_info.name)) && map_info.key_size == 4 &&
            map_info.value_size == 4) {
            xsks_map_fd = fd;
            break;
        }
...
static void apply_setsockopt(struct xsk_socket_info *xsk)
{
    ...
    sock_opt = 1;
    if (setsockopt(xsk_socket__fd(xsk->xsk), SOL_SOCKET, SO_PREFER_BUSY_POLL, (void *)&sock_opt, sizeof(sock_opt)) < 0)
        exit_with_error(errno);

    sock_opt = 20;
    if (setsockopt(xsk_socket__fd(xsk->xsk), SOL_SOCKET, SO_BUSY_POLL, (void *)&sock_opt, sizeof(sock_opt)) < 0)
        exit_with_error(errno);

    sock_opt = opt_batch_size;
    if (setsockopt(xsk_socket__fd(xsk->xsk), SOL_SOCKET, SO_BUSY_POLL_BUDGET, (void *)&sock_opt, sizeof(sock_opt)) < 0)
        exit_with_error(errno);
}
...
int main(int argc, char **argv)
{
    ...
    if (opt_reduced_cap) {
        ...
        if (capget(&hdr, data) < 0) {
            ...
            fprintf(stderr, "Capabilities EFF %x Caps INH %x Caps Per %x\n", data[0].effective, data[0].inheritable,
                    data[0].permitted);
            fprintf(stderr, "Capabilities EFF %x Caps INH %x Caps Per %x\n", data[1].effective, data[1].inheritable,
                    data[1].permitted);
        }
    } else {
